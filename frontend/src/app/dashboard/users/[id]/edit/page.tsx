...
export default function EditUserPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setForm({
        ...
        location_permissions: userRes.location_permissions,
        account_holder_name: bank.account_holder_name || '',
        account_number: bank.account_number || '',
        bank_name: bank.bank_name || '',
        bank_identification_code: bank.bank_identification_code || '',
        branch: bank.branch || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await api.put(`/users/${params.id}`, form, token);
    ...
    <>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-muted mt-1">{form.first_name as string} {form.last_name as string}</p>
        </div>
        <Link href={`/dashboard/users/${params.id}`} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-muted bg-card-bg border border-border rounded-xl hover:bg-background hover:text-foreground transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={sectionClass}>
              <div className="flex items-center gap-3 mb-5 pb-4 border-b border-border/50">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted">Role assignment and permissions</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
              <Link href={`/dashboard/users/${params.id}`} className="px-5 py-2.5 text-sm font-medium border border-border rounded-xl hover:bg-background transition-colors text-muted hover:text-foreground">
                Cancel
              </Link>
              <button
                type="submit"
          ...
          <div className="space-y-4">
            <div className="bg-card-bg rounded-2xl border border-border/60 p-5">
              <h3 className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">Summary</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between py-2 px-3 bg-primary/5 rounded-lg">
                  <span className="text-sm text-muted">Name</span>
                  <span className="text-sm font-medium text-foreground">{form.first_name as string} {form.last_name as string}</span>
                </div>
                <div className="flex items-center justify-between py-2 px-3 bg-primary/5 rounded-lg">
                  <span className="text-sm text-muted">Role</span>
                  <span className="text-sm font-medium text-foreground">{roles.find((r) => r.id === (form.role_id as number))?.name || '-'}</span>
                </div>
              </div>
            </div>
          </div>
